...
The `elog2cbs` method is a fast implementation for converting the event log into a customer-by-sufficient-statistic summary table, which is the required format for applying BTYD and BTYDplus models. Note, that compared to `BTYD::dc.ElogToCbsCbt` this implementation also adds a summary statistic for estimating regularity (`litt`), plus aggregates sales, if these are present in the event log (`sales`). 
...
By specifying an end date for the calibration period (`T.cal`), it is possible to generate summary statistics split into calibration and holdout period, which can be useful for evaluating forecasting accuracy for a given dataset.
...
FIXME For one, we can convert the logs into weekly and/or daily incremental number of repeat transactions, and have these plotted. Further, we can plot the transaction dates of some randomly sampled customers, to get a feeling for the various timing patterns.
