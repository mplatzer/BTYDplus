...
set.seed(123)
data("groceryElog")
grocerySubset <- groceryElog[groceryElog$cust %in% sample(unique(groceryElog$cust), 20), ]
grocerySubset <- grocerySubset[grocerySubset$date <= "2006-12-31", ]
plotSampledTimingPatterns(grocerySubset, T.cal = "2006-12-31")
```
...
knitr::kable(head(cdnowElog[, c("cust", "date", "sales")], 6), caption = "First Entries of CDNow Event Log")
```
...
plotSampledTimingPatterns(groceryElog, n = 20)
```
