...
plot(weekly_inc_total, typ = "l", frame = FALSE,
     ylab = "", xlab = "", main = "Incremental Transactions")
...
plot(weekly_cum_total, typ = "l", frame = FALSE,
     ylab = "", xlab = "", main = "Cumulative Transactions")
...
The NBD model by @ehrenberg1959pattern assumes a heterogenous, yet constant purchasing process, with its purchase rate $\lambda$ being $Gamma(r, \alpha)$-distributed across customers.
...
With the mean of the Gamma distribution being $r / \alpha$, this yields a mean estimate for $\lambda$ of `r round(params.nbd[1] / params.nbd[2], 2)`, and thus a mean intertransaction time $1 / \lambda$ of `r round(params.nbd[2] / params.nbd[1], 2)` weeks.

Let's next estimate the number of transactions for the year 2007, and compare it to the actual observed transactions.

```{r}
cbs$x.star.nbd <- nbd.ConditionalExpectedTransactions(params.nbd, T.star = 52, cbs$x, cbs$T.cal)
c(`Actual` = sum(cbs$x.star), `NBD Estimate` = sum(cbs$x.star.nbd))
```

The NBD model overforecasts in this case the actual number of transaction by `r paste0(round(100 * sum(cbs$x.star.nbd) / sum(cbs$x.star) - 100, 1), "%")`, which can be explained by the lack of a dropout process. All customers are assumed to remain as active in 2007 as they have been in 2006, whereas in reality the weekly incremental transaction counts show a clear downward trend.
...
```{r}
#head(cbs[rev(order(cbs$x.star.nbd)), c("T.cal", "x", "T.star", "x.star", "x.star.nbd")], 5) # 5 most valuable customers
```
