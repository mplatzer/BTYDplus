...
Let's next estimate the number of transactions for the year 2007, and compare it to the 


```{r}
cbs$xstar.nbd <- nbd.ConditionalExpectedTransactions(params.nbd, T.star = 52, cbs$x, cbs$T.cal)
c(`Actual` = sum(cbs$x.star), `NBD Estimate` = sum(cbs$xstar.nbd))
head(cbs[rev(order(cbs$xstar.nbd)), ], 5) # 5 most valuable customers
```
