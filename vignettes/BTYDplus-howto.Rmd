...
```{r, fig.show="hold", fig.width=7, fig.height=2.5, fig.cap="Figure 2: Weekly Trends"}
data("groceryElog")
...
```{r, fig.show="hold", fig.width=7, fig.height=2.5, fig.cap="Figure 3: Diagnostic Plots for Estimating Regularity"}
data("groceryElog")
op <- par(mfrow = c(1, 2))
...
pnbd.draws <- pnbd.mcmc.DrawParameters(cbs) # runs for ~13secs on a 2015 MacBook Pro
...
```{r, fig.show="hold", warning=FALSE, fig.width=7, fig.height=3, fig.cap="Figure X: MCMC traces and parameter distributions of cohort-level parameters"}
...
```{r, fig.show="hold", warning=FALSE, fig.width=7, fig.height=3, fig.cap="Figure X: MCMC traces and parameter distributions of individual-level parameters for customer 1"}
...
```{r, fig.show="hold", fig.width=7, fig.height=3, fig.cap="Figure X: Diagnostic Plots"}
op <- par(mfrow = c(1, 2))
...
draws.m1 <- abe.mcmc.DrawParameters(cdnowCbs, 
              mcmc = 5000, burnin = 5000, chains = 4) # runs for ~2mins on a 2015 MacBook Pro
...
```{r, fig.caption = "Figure X: Distribution of regularity parameter k"}
# generate parameter draws
pggg.draws <- pggg.mcmc.DrawParameters(cbs) # runs for ~2.5mins on a 2015 MacBook Pro
# generate draws for holdout period
...
```{r, fig.caption = "Figure X: Distribution of regularity parameter k"}
apply(as.matrix(pggg.draws$level_2), 2, mean)
pggg.plotRegularityRateHeterogeneity(pggg.draws, title = "")
```

When checking the estimated heterogeneity parameters, we see that k is clearly estimated larger than 1, indicating the regularity is present. The above plot furhter shows that there is significant variation in the regularity across customers.
...
When comparing forecasting accuracy at individual level, we can see a lift of `r paste0(round(100*unname(lift <- 1 - mae[1]/mae[2]), 1), "%")` for the given dataset, due to the detected regularity.
