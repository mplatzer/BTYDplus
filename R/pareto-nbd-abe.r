  ...
  run_single_chain <- function(chain_id, data) {
    ...
    for (step in 1:(burnin+mcmc)) {
      ...
      if ((step-burnin)>0 & (step-1-burnin)%%thin==0) {
        ...
        level_2_draws[idx,] <- c(level_2$beta, level_2$gamma[1,1], level_2$gamma[1,2], level_2$gamma[2,2])
      }
    }
...
abe.test <- function() {
  ...
  params$gamma <- matrix(c(0.2, 0, 0, 0.33), ncol=2)
  set.seed(1)
  n <- 5000
  data <- abe.GenerateData(n, T.cal=32, T.star=32, params)$cbs
  draws <- abe.mcmc.DrawParameters(data, thin=1)

  plot(draws$level_2, density=F)
  
  mus <- sapply(draws$level_1, function(draw) as.matrix(draw[, "mu"]))
  plot(apply(log(mus), 1, max), typ="l")
  plot(apply(log(mus), 1, min), typ="l")
