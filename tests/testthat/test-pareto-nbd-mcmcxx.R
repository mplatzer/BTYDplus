...
test_that("Pareto/NBD MCMC", {
    ...
    expect_less_than(ape(params$r, est$r), 0.2)
    expect_less_than(ape(params$alpha, est$alpha), 0.2)
    expect_less_than(ape(params$s, est$s), 0.2)
    expect_less_than(ape(params$beta, est$beta), 0.4)  # beta hard to identify
