...
test_that("MBG/NBD", {
  ...
  expect_silent(mbgnbd.PlotTrackingInc(params, cbs$T.cal, max(cbs$T.cal+cbs$T.star), elog2inc(elog)))
  expect_silent(mbgnbd.PlotTrackingCum(params, cbs$T.cal, max(cbs$T.cal+cbs$T.star), elog2cum(elog)))
  mat <- mbgnbd.PlotFrequencyInCalibration(params, cbs, 7)
